...
//Bit Sync Interrupt; PIN D3
ISR(INT1_vect)
{
  EIMSK &= !(1 << INT1);
  startTimer(); //reset timer to reinitialize bit sync
...
int main(void)
{
  ...
  uint8_t syncCounter = 0x00;

  //###############   PIN INTERRRUPT SETUP  #####################
  DDRD = DDRD | (0<<PORTD2)| (0<<PORTD3);  //D2 Interrupt set to input
  EICRA = 1<<ISC11 | 1<<ISC10 | 0<<ISC01 | 1<<ISC00; //set INT0 to trigger on logic change [0 1]
